...
void twophasekOmegaCBuoy<BasicTurbulenceModel>::correctNut()
{
    this->nut_ = k_/
    (
...
template<class BasicTurbulenceModel>
twophasekOmegaCBuoy<BasicTurbulenceModel>::twophasekOmegaCBuoy
(
    const alphaField& alphaw,
    const rhoField& rho,
...
template<class BasicTurbulenceModel>
bool twophasekOmegaCBuoy<BasicTurbulenceModel>::read()
{
...
template<class BasicTurbulenceModel>
void twophasekOmegaCBuoy<BasicTurbulenceModel>::correct()
{
    ...
    // Local references
    const alphaField& alphaw = this->alpha_;
    const alphaField alphas = 1 - alphaw;
    ...
    (
    sigmad_*pos(0.15-alphas)*pos(alphadCheck_)*(alphadCheck_)/omega_
    );
    ...
    (
      ...
      + fvm::Sp((C4om_*KE4_*ESD5CBuoy_*nut/k_), omega_)
    );
    if (writeTke_)
    {
        #include "writeTKEBudget_kOmegaCBuoy.H"
    }
    omegaEqn.ref().relax();
